...
class FilePathDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Automatic SQL detection: if path consists only of digits, it is a SQL row index
        self.use_sql = len(self.data_list) > 0 and self.data_list[0][0].isdigit()
        if self.use_sql:
            ...
            print(f"[SQL] Initialized SQL Dataset with parquet source: {root_path}")
            print(f"[SQL] Mapped speakers: {self.speaker_map}")
    ...
    def _load_tensor(self, data):
        ...
        if hasattr(self, "speaker_map"):
            ...
            try:
                ...
                # Fallback if speaker mapping was not initialized or not digits
                unique_speakers = sorted(list(set(d[2] for d in self.data_list)))
                self.speaker_map = {name: idx for idx, name in enumerate(unique_speakers)}
                speaker_id = self.speaker_map.get(speaker_id, 0)
        ...
        if self.use_sql:
            ...
            if res is None or res[0] is None:
                raise ValueError(f"[SQL] Row index {row_idx} not found in parquet files at {self.root_path}")
            audio_bytes = res[0]
