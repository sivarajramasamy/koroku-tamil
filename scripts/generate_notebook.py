 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# 4. Apply custom SQL-loading patches\n",
    "!cp training/patches/utils.py StyleTTS2/utils.py\n",
    "!cp training/patches/meldataset.py StyleTTS2/meldataset.py\n",
    "\n",
    "# 5. Install training framework requirements\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "print(\"Piping Tamil Parquet data onto fast local scratch storage...\")\n",
    "start_time = time.time()\n",
    "\n",
    "for f in os.listdir(DRIVE_SRC):\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "print(con.execute(\"DESCRIBE SELECT * FROM read_parquet('/content/local_tamil_parquet/*.parquet') LIMIT 1\").df())\n",
    "\n",
    "print(\"\\nGenerating Tamil Out-Of-Domain texts for Stage 2...\")\n",
    "rows = con.execute(\"SELECT text FROM read_parquet('/content/local_tamil_parquet/*.parquet') WHERE text IS NOT NULL LIMIT 500\").fetchall()\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "print(\"Pretrained base model weights downloaded successfully.\")"
   ]
  },
  {
   "cell_type": "markdown",
